...
FlightPilot::FlightPilot(const ros::NodeHandle &nh, const ros::NodeHandle &pnh)
  ...
  rgb_pub_ = img_it_->advertise(rgb_topic_, 1);
  ROS_INFO_STREAM("Publishing rgb image on: " << rgb_topic_);
...
void FlightPilot::poseCallback(const nav_msgs::Odometry::ConstPtr &msg) {
  ...
  if (rgb_camera_ && rgb_pub_) {
    cv::Mat img;
    if (rgb_camera_->getRGBImage(img)) { // RGBCamera API same as camera.cpp
      std_msgs::Header header;
      header.stamp = ros::Time::now();
      header.frame_id = "hummingbird/camera"; // adjust if needed
