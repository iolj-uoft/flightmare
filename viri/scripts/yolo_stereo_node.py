...
from vision_msgs.msg import Detection2DArray, Detection2D  # <-- NEW IMPORTS
...
def main():
    ...
    # Topic to publish the target coordinates to your stereo estimator
    ...
    # NEW: Publisher using vision_msgs to support synchronized timestamps
    bbox_pub = rospy.Publisher(bbox_topic, Detection2DArray, queue_size=1)
    ...
    def sync_callback(msg_left, msg_right):
        ...
        # 4. PUBLISH COORDINATES (Using Detection2DArray)
        if x_l != -1.0 and x_r != -1.0:
            det_array = Detection2DArray()
            
            # ==========================================
            # THE MAGIC FIX: Attach the original image timestamp
            # ==========================================
            det_array.header = msg_left.header 
            
            det = Detection2D()
            # We pack the stereo coordinates into the standard bbox fields 
            # so the C++ node can easily unpack them without custom messages.
            det.bbox.center.x = (x_l + x_r) / 2.0
            det.bbox.center.y = (y_l + y_r) / 2.0
            det.bbox.size_x = x_r - x_l
            det.bbox.size_y = y_r - y_l
            
            det_array.detections.append(det)
            bbox_pub.publish(det_array)
        ...
        out_msg_left.header = msg_left.header 
